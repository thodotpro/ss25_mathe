...
class PlottingTool2D:
    ...
    def run(params: dict):
        ...
        filename = params.get("filename", os.path.join("output/plot", "plot2d.png"))
        ...
        plt.close()
        return f"2D plot saved as {filename}."
...
class PlottingTool3D:
    ...
    def run(params: dict):
        ...
        # Ensure output/plot directory exists
        os.makedirs("output/plot", exist_ok=True)
        filename = params.get("filename", os.path.join("output/plot", "plot3d.png"))
        ...
        plt.savefig(filename)
        plt.close(fig)
        return f"3D plot saved as {filename}."